...
if (Yii::app()->user->CF_STATUS != 1) {
    $main_column_agent = Yii::app()->cache->get('main_column_agent');
    if ($main_column_agent === false) {
        $main_column_agent = array(array(
                                   ...
                                   array(
                                       ...
                                       'filter' => CHtml::listData(DictDemandStatus::model()->actual()->findAll('id in (1,2,3,4,9)'), 'id', 'c_name'),
                                   ),
                                   array(
        ...
        Yii::app()->cache->set('main_column_agent', $main_column_agent, 3600, $dependency);
    }
    ...
    $main_column_manager = Yii::app()->cache->get('main_column_manager');
    if ($main_column_manager === false) {
        $main_column_manager = array(
            ...
            array(
                ...
                'name' => 'is_demand_status',
                'value' => '$data->DemandStatus->c_name',
                'filter' => CHtml::listData(DictDemandStatus::model()->actual()->findAll('id in (-1,1,2,3,4,9)'), 'id', 'c_name'),
            ),
            array(
                'header' => 'Комментарий БС',
                'value' => '$data->t_user_comment',
        ...
        Yii::app()->cache->set('main_column_manager', $main_column_manager, 3600, $dependency);
    }
    $b = $this->widget('application.zii.widgets.grid.DefaultGridView', array(
                                                                            'id' => 'object-resto-grid',
